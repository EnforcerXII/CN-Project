...
def receive(sock, username):  # runs in a sep thread
...
def main():
    ...
    threading.Thread(
        target=receive, args=(sock, username), daemon=True
    ).start()  # arg must be tuple
    threading.Thread(
        target=heartbeat, args=(sock,), daemon=True
    ).start()  # daemon-background thread that automatically stops when main program ends
